...
use Queue\RedisQueue\ReQueue\Failure\FailureInterface;
use Queue\RedisQueue\ReQueue\Worker;
use Queue\RedisQueue\ReQueue\Failure\FailureRedis;
...
class Failure
{
	 ...
	 * @param \Queue\RedisQueue\ReQueue\Worker $worker Instance of Worker that was running this job when it failed.
	...
	public static function create($payload, \Exception $exception, Worker $worker, $queue)
	{
		...
		new FailureRedis($payload, $exception, $worker, $queue);

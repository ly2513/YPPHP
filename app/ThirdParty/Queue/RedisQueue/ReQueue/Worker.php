...
use Queue\RedisQueue\ReQueue\Stat;
use Queue\RedisQueue\ReQueue\Event;
use Queue\RedisQueue\ReQueue\Job;
use Queue\RedisQueue\ReQueue\Job\Status;
use Queue\RedisQueue\ReQueue\Job\DirtyExitException;
use Queue\RedisQueue\ReQueue\Log;
...
class Worker
{
    ...
    public function work($interval = 5)
    {
        ...
        while (TRUE) {
            ...
            if (!$job) {
                ...
                //$this->log->writeLog('Sleeping for ' . $interval);
    ...
    public function perform(Job $job)
    {
        try {
            ...
            $job->perform($job->payload['args']['jobId']);
            //$job->perform();
        } catch (Exception $e) {

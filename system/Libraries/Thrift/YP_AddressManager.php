...
class YP_AddressManager
{
    ...
    public static function getOneAddress($key)
    {
        ...
        if (empty($address_list)) {
            throw new \Exception("No avaliable server node! service_name:$key allAddress:[" . implode(',',
                    self::$config[$key]) . '] badAddress:[' . implode(',', $bad_address_list) . ']');
        }
    ...
    public static function checkConfigMd5()
    {
        ...
        // 清空badAddressList
        self::getMutex();
        $ret = shm_put_var(self::getShmFd(), self::SHM_BAD_ADDRESS_KEY, []);
        self::releaseMutex();
        if ($ret) {
            // 写入md5值
            self::getMutex();
